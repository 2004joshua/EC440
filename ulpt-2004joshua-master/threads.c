...
static void schedule(int signal) { 
  ...
  for(int i = 0; i < MAX_THREADS; i++) { 
    ...
    if(threads[next].status == __READY) {
      CurrentThread = next; 
      found = 1; 
      break; 
    }
  ...
  if(prev->status != __EXITED && prev->status != __BLOCKED) {
    if(setjmp(prev->context) == 0) {
      prev->status = __READY;
      current->status = __RUNNING;
      longjmp(current->context,1);
    }
  }
  // case 2: previous was blocked
  else if(prev->status == __BLOCKED){
    if(setjmp(prev->context) == 0) {
      longjmp(current->context, 1);
    }
    else {
...
static void scheduler_init() {
  ...
  for(int i = 1; i < 128; i++) {
    threads[i].status = __UNINITIALIZED;
  }

  threads[0].status = __RUNNING;
...
int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine) (void *), void *arg) {
  ...
  */
  ...
  for(int i = 1; i < MAX_THREADS; i++) {
    if(threads[i].status == __UNINITIALIZED) {
      idx = i; 
      break; 
    }
  }
...
void pthread_exit(void *retval) {
  ...
  */
  ...
  threads[CurrentThread].status = __EXITED;
...
int pthread_join(pthread_t thread, void **retval) {
  ...
  for(int i = 0; i < MAX_THREADS; i++){
    if(threads[i].status != __UNINITIALIZED && threads[i].id == thread){
      while(threads[i].status != __EXITED){
        schedule_wrap(0); 
      }
...
int pthread_mutex_lock(pthread_mutex_t *mutex) {
	...
	while(_mut.vars._lock == 1) {
		...
		threads[(int)pthread_self()].status = __BLOCKED;
		schedule_wrap(0);
	}
...
int pthread_barrier_init(pthread_barrier_t *restrict barrier, const pthread_barrierattr_t *restrict attr, unsigned count) {
	...
	_barrier.vars.id = barrier_idx;
	barrier_idx++; 	

	for(int i = 0; i < MAX_THREADS; i++) {
		_barrier.vars.thread_queue[i] = -1; 
	}	
...
int pthread_barrier_destroy(pthread_barrier_t *barrier) {
	...
	_barrier.vars.count = -1; 
	_barrier.vars.limit = -1;
...
int pthread_barrier_wait(pthread_barrier_t *barrier) {
	...
	int idx = _barrier.vars.count++;
	_barrier.vars.thread_queue[idx] = (int)pthread_self(); 
	...
	if(_barrier.vars.count < _barrier.vars.limit){
		...
		*barrier = _barrier.barrier; 
		...
		for(int i = 0; i < _barrier.vars.limit; i++) {
			int tid = _barrier.vars.thread_queue[i];
		        if(tid != -1) {
				threads[tid].status = __READY; 
			}	
			_barrier.vars.thread_queue[i] = -1; 
		}
